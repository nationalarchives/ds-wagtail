...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if options.get("missing_2fa"):
            ...
            self.stdout.write(self.style.SUCCESS(f"Found {len(matches)} user(s) without 2FA devices."))
        ...
        if options.get("missing_recovery_codes"):
            ...
            matches = find_matching_users(
                lambda u: django_otp.user_has_device(u, confirmed=True)
                and not __import__(
                    "django_otp.plugins.otp_static.models", fromlist=["StaticDevice"]
                ).StaticDevice.objects.filter(user=u).exists()
            )
            if not matches:
                self.stdout.write(self.style.WARNING("No users found who have 2FA but lack recovery codes."))
                logger.info("No users found who have 2FA but lack recovery codes.")
                return
            for u in matches:
                self.stdout.write(f"- {u.email or u.username} (ID: {u.pk})")
            self.stdout.write(self.style.SUCCESS(f"Found {len(matches)} user(s) with 2FA but no recovery codes."))
