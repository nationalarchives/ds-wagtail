...
class CatalogueSearchEndToEndTest(EndToEndSearchTestCase):
    ...
    def test_render_invalid_date_range_all_dates(self, mock_get_random_string):
        ...
        self.assertSearchWithinOptionRendered(content)
        expected_created_dates_error_msg = '<ul class="errorlist nonfield"><li>There is a problem. Start date cannot be after end date.</li></ul>'
        self.assertContains(response, expected_created_dates_error_msg)
        expected_opening_dates_error_msg = '<ul class="errorlist nonfield"><li>There is a problem. Start date cannot be after end date.</li></ul>'
