...
def get_DeptName(record: dict, surrogate: List) -> str:
    return(get_Dept(record.reference_number, "deptname"))

def get_DeptUrl(record: dict, surrogate: List) -> str:
    return(get_Dept(record.reference_number, "depturl"))
...
def get_FoiUrl(record: dict, surrogate: List) -> str:
    return f"{settings.BASE_DISCOVERY_URL}/foirequest?reference={record.reference_number}"
...
def get_RecordUrl(record: dict, surrogate: List) -> str:
    return f"{settings.BASE_TNA_URL}/catalogue/id/{record.iaid}/"
...
deliveryOptionsTags = {
    ...
    '{WebsiteUrls}':                        get_WebsiteUrls,
    '{WebsiteUrl}':                         get_WebsiteUrl,
...
def html_replacer(string: str, record: dict, surrogate_data: List) -> str:
    ...
    for s in subs:
        try:
            ...
            string = string.replace(s, func(record, surrogate_data))
        except:
            raise
    return string

def html_builder(delivery_option_data: Union[List, str], record_data: Dict, surrogate_data: List = [], dcs: bool = False) -> str:
    html = ""
...
def orderbuttons_builder(delivery_option_data: List, record_data: Dict, surrogate_data: List) -> List:
    for item in delivery_option_data:
        item['href'] = html_builder(item['href'], record_data, surrogate_data=surrogate_data)
        item['text'] = html_builder(item['text'], record_data, surrogate_data=surrogate_data)
    return delivery_option_data

# Specific pre-processing for the basket limit data
def basketlimit_builder(delivery_option_data: Union[List, str], record_data: Dict) -> str:
    return html_builder(delivery_option_data, record_data)

# Specific pre-processing for the expand link data
def expandlink_builder(delivery_option_data: Union[List, str], record_data: Dict) -> str:
    return html_builder(delivery_option_data, record_data)

# Specific pre-processing for the description data
def description_builder(delivery_option_data: Union[List, str], record_data: Dict, surrogate_data: List) -> str:
    if distressing_content_match(record_data.reference_number):
        return html_builder(delivery_option_data, record_data, surrogate_data=surrogate_data, dcs=True)
    
    return html_builder(delivery_option_data, record_data, surrogate_data=surrogate_data)

# Specific pre-processing for the supplemental data
def supplemental_builder(delivery_option_data: Union[List, str], record_data: Dict, surrogate_data: List) -> str:
    return html_builder(delivery_option_data, record_data, surrogate_data=surrogate_data)

# Specific pre-processing for the heading
def heading_builder(delivery_option_data:str, record_data: Dict, surrogate_data: List) -> str:
    return html_builder(delivery_option_data, record_data, surrogate_data=surrogate_data)
...
def construct_delivery_options(doptions: list, record: dict) -> dict:
    ...
    do['surrogate'], do['av_media'] = surrogate_link_builder(doptions[0]['surrogateLinks'])
    
    do['index'] = doptions[0]['options']
